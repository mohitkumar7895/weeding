...
export async function GET(request: Request) {
  try {
    ...
    const [paymentsResult, commissionResult, refundsResult, payoutsResult, disputesResult] = await Promise.all([
      safeSelect<any[]>(
        `SELECT status, SUM(amount) as total_amount, COUNT(*) as count FROM payment_transactions WHERE 1=1${dateFilter} GROUP BY status`,
        params
      ),
      safeSelect<any[]>(`SELECT SUM(commission_amount) as total FROM commission_records WHERE 1=1${dateFilter}`, params),
      safeSelect<any[]>(
      ...
      safeSelect<any[]>(`SELECT COUNT(*) as count FROM disputes WHERE status = 'OPEN'${dateFilter}`, params),
    ]);
